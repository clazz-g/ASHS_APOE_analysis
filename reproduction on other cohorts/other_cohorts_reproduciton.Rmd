...
```{r, include=F}
hippo_WRAP<-read_delim(paste0(config$wrap_dataset_cleaned, "hippocampus_dataset.csv"),col_select = -1)

cog_WRAP<-read_delim("G:/My Drive/Data/WRAP/RobustNormZScores.csv")%>%
  dplyr::select(WRAPNo, VisNo, RNZ_TrailMakingTestTrailATime_trla)


ptau_WRAP<-read_delim(paste0(config$wrap_dataset_cleaned, "ptau217_dataset.csv"), col_select = -1)

centiloids_WRAP<-read_delim(paste0(config$wrap_dataset_cleaned, "centiloids_PIB_dataset.csv"), col_select = -1)
...
time_between_scans<-WRAP%>%
  dplyr::select(subject, days_since_baseline, Visit)%>%
  pivot_wider(names_from = Visit, values_from = days_since_baseline, names_prefix = "days_scan_")%>%
  mutate(diff_mri = days_scan_2 - days_scan_1)%>%
  dplyr::select(subject, diff_mri)%>%
  mutate(diff_mri = diff_mri/365.25)
...
desc_table<-tbl_summary(WRAP_descriptive,
            ...
            label= list(kids_total = "Number of childbirths",
                     ...
                     age_at_acquisition = "Age at first scan, Mean (SD)",
...
fit_wrap<-lmer(vol_HIPP ~ APOE_binary*diff_mri*kids_total + scale(rbm_icv)  + age_baseline + education + (1|subject), data=WRAP2)
summary(fit_wrap)
...
-   Overall results are similar but not significant
