...
Parity, APOE-ε4, and time showed a significant three-way interaction for total HV (β = `r tidy(mod_ashs_APOE)$estimate[11]`, SE = `r tidy(mod_ashs_APOE)$std.error[11]`, p = `r tidy(mod_ashs_APOE)$p.value[11]`), indicating that higher parity was associated with steeper decline in HV among APOE-ε4 carriers.

Stratifying by APOE-ε4 status yielded a significant interaction between parity and time in carriers only (β = `r tidy(mod_ashs_APOE_A1)$estimate[7]`, SE = `r tidy(mod_ashs_APOE_A1)$std.error[7]`, p = `r tidy(mod_ashs_APOE_A1)$p.value[7]`).
...
Parity showed a near significant association with YKL-40 in APOE4 carriers, independent of time (β= `r tidy(yk402)$estimate[2]`, SE = `r tidy(yk402)$std.error[2]`, p= `r tidy(yk402)$p.value[2]`). 

## CVR
```{r}
# framingham<-read_delim(paste0(config$alfa_dataset_raw, "CVR/Framingham_APlus_V1_20250730(in).csv"), 
#     delim = ";", escape_double = FALSE, trim_ws = TRUE)%>%
#   dplyr::select(IdParticipante, Risk)%>%
#   mutate(Risk = as.numeric(Risk))%>%
#   rename(subject = IdParticipante)
# 
# data_ashs_female_cvr<-data_ashs_female%>%
#   left_join(., framingham)
# 
# mod_ashs_APOE<-lmer(vol_HIPP ~  kids_total*APOE_binary*diff_mri +age_v1 + scale(vol_tiv) + education + Risk +(1|subject), data = data_ashs_female_cvr)
# summary(mod_ashs_APOE)
```
