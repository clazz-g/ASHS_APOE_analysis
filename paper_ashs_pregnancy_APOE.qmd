...
Lee and colleagues @lee2024 propose that this "parity paradox" could be reconciled using the Healthy cell bias hypothesis. This theory suggests that reproductive experiences (and their associated hormonal changes) may be beneficial in a healthy brain but detrimental in an unhealthy or at-risk brain.. Using wildtype and APOE4 models middle aged female rats, they show that APOE status modifies the effect of parity on neurogenesis and HV. Parity was associated with neurogenesis in wildtype rats, particularly in the ventral dentate gyrus, but decreased neurogenesis in APOE4 rats. In addition to this, parous APOE4 rats showed a spatial solving strategy which indicated a decreased recruitment of the hippocampus. To our knowledge, no study has explored this interaction of APOE4 and parity on hippocampal structure in humans.
...
Lastly, we conducted similar linear regression using inflammatory biomarkers as outcomes to investigate their role in the observed association. These analyses were normalised using box-cox transformation to ensure that the distribution of residuals remained normal.
...
data_table<-data_naked%>%
  ...
  left_join(., diff_mri)%>%
  dplyr::select(age_v1, kids_total, education, APOE_binary, CSF_Abeta_ratio_V1, vol_HIPP_1, vol_HIPP_2, diff_mri, attention_V1, attention_V2, executive_V1, executive_V2, memory_V1, memory_V2, language_V1, language_V2, visual_V1, visual_V2, vol_tiv_1)
...
```
The interaction between parity, APOE4 and time yielded a weak trend which was not present after FDR correction. 
...
gfap<-lmer(CSF_GFAP ~ kids_total*APOE_binary*biomarkers_time_diff + age_v1  + CSF_Abeta_ratio+ education +(1|subject), data = data_biomarkers_long_female)
...
gfap2<-lmer(CSF_GFAP_transformed ~ kids_total*biomarkers_time_diff*APOE_binary + age_v1  + CSF_Abeta_ratio+ education +(1|subject), data = data_biomarkers_long_female)
summary(gfap2) 
plot(gfap2)
...
strem<-lmer( CSF_sTREM2 ~ kids_total*APOE_binary*biomarkers_time_diff + age_v1  + CSF_Abeta_ratio+ education +(1|subject), data = data_biomarkers_long_female)
...
strem2<-lmer(CSF_sTREM2_transformed ~ kids_total*APOE_binary*biomarkers_time_diff + CSF_Abeta_ratio +age_v1  + education +(1|subject), data = data_biomarkers_long_female)
summary(strem2) 
plot(strem2)
...
yk40<-lmer( CSF_YKL40 ~ kids_total*APOE_binary*biomarkers_time_diff + age_v1  + CSF_Abeta_ratio+ education +(1|subject), data = data_biomarkers_long_female)
...
yk402<-lmer(CSF_YKL40_transformed ~ kids_total*APOE_binary*biomarkers_time_diff + CSF_Abeta_ratio+ age_v1  + education +(1|subject), data = data_biomarkers_long_female)
summary(yk402) 
plot(yk402)
...
No association between was found between the 3-way interaction of parity, APOE4 and time, and any of the biomarkers assessed.


# Discussion

Our study aimed to investigate the impact of parity on hippocampal subfield volumes and relation to the APOE4 allele in CU post-menopausal women at risk of AD. Our results suggest that parity and AD-related genotype interact in women to impact hippocampal volume loss. Specifically, APOE4 carriers showed an association between parity and steeped HV loss, but the opposite was true in non carriers. Stratified analyses revealed that the interactive effects of parity and time were only significant in APOE4 carriers. These interactive effects were not explained by potential confounding factors such as AB accumulation or other reproductive variables. Exploratory analyses of hippocampal subfields showed a significant association between the 3-way interaction and the CA1 and DG after adjusting for AB load. No association with cognition was found. Together, these findings highlight that being a carrier of the APOE4 allele may... 


The main finding of our study was that APOE4 carriers showed an association between higher parity and steeper hippocampal decline. Our results evoke that previous parity may confer a deleterious effect on HV, a hallmark marker of early AD-related neurodegeneration, in carriers on the APOE4 allele. Although in our cohorts, APOE4 carriers showed significantly higher accumulations of AB load, adjusting for this variable did not seem to explain the observed effect, Moreover, this effect was also seen reproducing the same analysis with AB negative subjects only. Therfore, our results do nto seem to reflect amyloid driven neurodegeneration, which can affect the hippocampaus in early stages of the disease (Raffa), but instead an independent pathway specific to APOE4 and parity. This is in line with the healthy cell bias hypothesis, since reproductive experience appears to be detrimental to APOE4 carriers only.


Exploratory analyses of hippocampal subfields shows this effect is driven by the CA1 and DG. The CA1 is one of the earliest impacted areas of the hippocampus in MCI/AD (sourcesss)
The involvment of the DG is of particular interest given its neurogenesis is higly impacted by parity in rodents. Despite the controversy behind the idea of neurogenesis in the adult DG...
...
Inflammation biomarkers did not show a similar association with the 3-way interaction. In their paper, Lee and colleagues porpose and test this theory by measuring IL-6 and... levels in their subjects. However, they were able to directly test presence of these markers in the regions of interest, whereas our measures were drawn from CSF, which would not be as accurate. 
