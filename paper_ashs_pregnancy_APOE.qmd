...
data_ashs_long<-ashs1%>%
  ...
  filter(subject!= 21136, #this is a very big outlier that increases 1cm3 of HV over visits
         ...
         subject != 12186, #cooks distance > 1
...
Parity was defined as the number of biological children and used as a continuous and dichotomous [parous vs nulliparous] variable in the main analyses. This information was self-reported by individuals during an interview with a clinician at the baseline visit.
...
CSF collection, processing, and storage in the ALFA+ study have been described previously @Milà-Alomà2020. Briefly, CSF Aβ42 and Aβ40 were measured with the exploratory Roche NeuroToolKit immunoassays (Roche Diagnostics International Ltd, Rotkreuz, Switzerland) on a cobas e 601 module. Measurements were performed at the Clinical Neurochemistry Laboratory, Sahlgrenska University Hospital, Molndal, Sweden. Aß accumulation (Aß+, Aß–) was estimated using CSF Aß42/40.
...
-   why lump together people with 3+ pregnancies instead of doing like in my previous paper?
...
cognition_wide<-read_excel(paste0(config$alfa_dataset_cognition, "PACCv1_to_v3(incomplete).xlsx"))%>%
  rename(subject = IdParticipante)%>%
...
data_table<-data_naked%>%
  ...
  mutate(Z_PACC_V1 = ifelse(is.na(Z_PACC_V2), NA, Z_PACC_V1),
         Z_PACC_V2 = ifelse(is.na(Z_PACC_V1), NA, Z_PACC_V2))%>%
  left_join(.,biomarkers_wide)%>%
  dplyr::select(age_v1, kids_total, APOE_binary, education, vol_tiv_1, vol_HIPP_1, vol_HIPP_2, , Z_PACC_V1, Z_PACC_V2, CSF_Abeta_ratio_V1, subject)%>%
  ...
  dplyr::select(age_v1, kids_total, education, APOE_binary, CSF_Abeta_ratio_V1, vol_HIPP_1, vol_HIPP_2,  Z_PACC_V1, Z_PACC_V2, vol_tiv_1)
...
desc_table<-tbl_summary(data_table,
            ...
            label= list(kids_total2 = "Number of childbirths",
                     ...
                     Z_PACC_V1 = "Baseline mPACC z-score, Mean (SD)", 
                     Z_PACC_V2 = "Follow-up mPACC z-score, Mean (SD)"),
...
As shown in Table 1, 39 (15.3%) participants had no previous childbirths, 54 (21.2%) had one previous childbirth, 126 (49.6%) had 2, 27 (10.6%) had 3, and only 8 (3%) had 4 or more. Age significantly differed depending on parity. No differences were observed among groups for Aβ, APOE-ε4 carrier status, or HV. 
...
## Cognitive trajectory - test adjusting for Abeta makes sense?
...
fit_cog_att<-lmer(attention ~ kids_total*time_diff*APOE_binary +age_v1  +education +(1 | subject), data_cog_composites)
summary(fit_cog_att)
...
Only trend between parity and time, and parity and APOE but no 3 way interaction, in attention. Nothing in the rest.

## Exploratory analysis of hippocampal subfields
need to recheck model residuals
```{r, include = F, warning=FALSE}

mod_ashs_CA1<-lmer(vol_CA1 ~  kids_total*diff_mri*APOE_binary +age_v1  + scale(vol_tiv) + education +(1|subject), data = data_ashs_female)
summary(mod_ashs_CA1)
influenceIndexPlot(mod_ashs_CA1)


mod_ashs_CA2<-lmer(vol_CA2 ~ kids_total*diff_mri*APOE_binary + age_v1  + scale(vol_tiv) + education +(1|subject), data = data_ashs_female)
summary(mod_ashs_CA2)
influenceIndexPlot(mod_ashs_CA2)

mod_ashs_CA3<-lmer(vol_CA3 ~  kids_total*diff_mri*APOE_binary +age_v1  + scale(vol_tiv) + education +(1|subject), data = data_ashs_female)
summary(mod_ashs_CA3)
influenceIndexPlot(mod_ashs_CA3)


mod_ashs_DG<-lmer(vol_DG ~  kids_total*diff_mri*APOE_binary +age_v1  + scale(vol_tiv) + education + (1|subject), data = data_ashs_female)
summary(mod_ashs_DG)
influenceIndexPlot(mod_ashs_DG)

mod_ashs_SUB<-lmer(vol_SUB ~  kids_total*diff_mri*APOE_binary +age_v1  + scale(vol_tiv) + education +(1|subject), data = data_ashs_female)
...
ggplot(forest_data, aes(x = estimate, y = region, color = APOE)) +
  ...
  labs(x = "Parity × Time Interaction",
...
ggplot(forest_data, aes(x = estimate, y = region, color = APOE)) +
  ...
  geom_text(aes(label = sig_label), 
            position = position_dodge(width = 0.5),
            vjust = -0.8, size = 4, show.legend = FALSE) +
  labs(x = "Parity × Time Interaction adjusted my AB load",
       y = "Hippocampal subfields",
       color = "APOE-e4") +
  th
```

## Calcharine structure

```{r, include = F}


```
No significant effect of the 3-way interaction was found in adjacent MTL structures.

## Exploratory analysis of hormonal variables
```{r}
data_ashs_female<-data_ashs_female%>%
...
## Exploratory analysis of inflammatory biomarkers - do I have to normalize all variables? 
...
data_biomarkers_long%>%
  drop_na(`CSF_IL-6`)%>%
  pull(`CSF_IL-6`)%>%
  descdist(., discrete = FALSE)



gfap<-lmer(CSF_GFAP ~ kids_total2*APOE_binary*biomarkers_time_diff + age_v1  + education +(1|subject), data = data_biomarkers_long_female)
...
gfap2<-lmer(CSF_GFAP_transformed ~ kids_total2*APOE_binary*biomarkers_time_diff + age_v1  + education +(1|subject), data = data_biomarkers_long_female)
summary(gfap2) 
plot(gfap2)
...
strem<-lmer( CSF_sTREM2 ~ kids_total2*APOE_binary*biomarkers_time_diff + age_v1  + education +(1|subject), data = data_biomarkers_long_female)
...
strem2<-lmer(CSF_sTREM2_transformed ~ kids_total2*APOE_binary*biomarkers_time_diff + age_v1  + education +(1|subject), data = data_biomarkers_long_female)
summary(strem2) 
plot(strem2)
qqnorm(residuals(strem2))

######
il6<-lmer( `CSF_IL-6` ~ kids_total2*APOE_binary*biomarkers_time_diff + age_v1  + education +(1|subject), data = data_biomarkers_long_female)
...
il62<-lmer(`CSF_IL-6_transformed` ~ kids_total2*APOE_binary*biomarkers_time_diff + age_v1  + education +(1|subject), data = data_biomarkers_long_female)
summary(il62) 
plot(il62)
...
yk40<-lmer( CSF_YKL40 ~ kids_total2*APOE_binary*biomarkers_time_diff + age_v1  + education +(1|subject), data = data_biomarkers_long_female)
...
yk402<-lmer(CSF_YKL40_transformed ~ kids_total2*APOE_binary*biomarkers_time_diff + age_v1  + education +(1|subject), data = data_biomarkers_long_female)
summary(yk402) 
plot(yk402)
...
s100<-lmer( CSF_S100 ~ kids_total2*APOE_binary*biomarkers_time_diff + age_v1  + education +(1|subject), data = data_biomarkers_long_female)
...
s1002<-lmer(CSF_S100_transformed ~ kids_total2*APOE_binary*biomarkers_time_diff + age_v1  + education +(1|subject), data = data_biomarkers_long_female)
summary(s1002) 
plot(s1002)
...
total_gray_v1<-read_excel(paste0(config$alfa_dataset_raw, "neuroimaging/freesurfer - aseg/bx_freesurfer7_aseg_ALFA_PLUS_V1_20231120_20250703_114141.xlsx"))%>%
  filter(measurement == "Volume_mm3")%>%
  filter(region %in% c("Left-Thalamus", "Right-Thalamus", "Left-Putamen", "Right-Putamen", "Left-Amygdala", "Right-Amygdala", "Left-Accumbens-area", "Right-Accumbens-area", "TotalGrayVol"))%>%
  dplyr::select(subject, value, region)%>%
  mutate(region_combined = gsub("^(Left-|Right-)", "", region))%>%
  group_by(subject, region_combined) %>%
  mutate(value=as.numeric(value))%>%
  summarise(value = sum(value))%>%
  pivot_wider(names_from = region_combined, values_from = value, names_prefix = "vol_")%>%
  mutate(Visit = 1, 
         subject=as.numeric(subject),
         diff_mri= 0)

total_gray_v2<-read_excel(paste0(config$alfa_dataset_raw, "neuroimaging/freesurfer - aseg/bx_freesurfer7_aseg_ALFA_PLUS_V2_20230518_20250703_114854.xlsx"))%>%
  filter(measurement == "Volume_mm3")%>%
  filter(region %in% c("Left-Thalamus", "Right-Thalamus", "Left-Putamen", "Right-Putamen", "Left-Amygdala", "Right-Amygdala", "Left-Accumbens-area", "Right-Accumbens-area", "TotalGrayVol"))%>%
  dplyr::select(subject, value, region)%>%
  mutate(region_combined = gsub("^(Left-|Right-)", "", region))%>%
...
total_gray_all<-bind_rows(total_gray_v1, total_gray_v2)

data_gray_all<-left_join(data_naked, total_gray_all)%>%
  filter(sex==2)%>%
  group_by(subject)%>%
  filter(n()==2)%>%
  ungroup()%>%
