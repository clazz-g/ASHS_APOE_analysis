...
Alzheimer's disease (AD) is a neurodegenerative disorder characterised by the accumulation of pathophysiological abnormalities leading to neurodegeneration and progressive cognitive and functional decline. Ageing, carring the apolipoprotein E E4 (APOE4) and having female chromosomal sex are three established non-modifiable risk factors for the typical late-onset of this disease @riedel2016. There is growing evidence showing an interaction between APOE4 and biological sex: whilst men carrying the APOE4 allele see their risk increase by ..., women's risk increase by .... Amongst APOE4 carriers, women exhibit an increased risk of conversion from healthy ageing to MCI, or from MCI to AD dementia between the ages of 65 to 75 [@Altmann2014; @Neu2017]. Factors underlying the interaction between APOE4 and sex are not well understood, warrating a deeper investigation of sex specific events which might interact with this phenomenon. In particular, pregnancy is a female-specific event with far reaching biological impacts that are not yet well-explored.

In humans, pregnancy is associated with drastic increases in sex-steroid levels [source] and a significant hippocampal volume loss @Hoekzema2017, which is detectable for up to at least 6 years post-partum in primiparous women @martínez-garcía2021. Interestingly, the dynamic volume changes seen during gestation are associated with the chnage curve seen in in free circulating sex-hormones (check source, carmona). In rodents, these changes are linked with decreased hippocampal neurogenesis during gestation and the postpartum period [8,9]. Crucially, the hippocampus is one of the first regions affected by AD, and whole hippocampal volume is a hallmark predictor of conversion from MCI to AD @apostolova2006. Therefore, this region seems of particular interest given it is affected by parity and is related to AD neurodegeneration.

Evidence on how previous pregnancies influence AD risk is conflicting. Amongst epidemiological studies that explored a potential association between parity and AD, some report a link between multiparity and greater cognitive decline @colucci2006, whilst other report no link or the opposite findings [@Fox2018; @Fox2013.; @wolfova2024] Additionally, a post-mortem histopathological study of 42 women, with clinical diagnosis spanning the spectrum from normal aging to definite AD, found that multiparity was associated with a greater number of neuritic plaques @Beeri2009a. However, analysis of Aβ-PET data from 248 women, including those with CU or clinically diagnosed MCI, found no association between Aβ burden and parity or multiparity [19]. 

These inconsistent findings may be understood through the lens of Brinton's "healthy cell bias" theory, which posits that estrogen's neuroprotective effects are contingent upon neuronal metabolic health. Under physiological conditions, estrogen enhances glucose metabolism and mitochondrial bioenergetics, which supports the energy-intensive process of Ca²⁺ homeostasis—mitochondria require substantial ATP to sequester excess cytoplasmic Ca²⁺ and maintain proper signaling dynamics. However, when metabolic injury is already present and Ca²⁺ regulation has been disrupted, estrogen exposure can paradoxically exacerbate neurodegeneration rather than prevent it. ApoE4 creates precisely this vulnerable metabolic state by impairing glucose metabolism and lysosomal function, essential for Ca²⁺ homeostasis [ https://doi.org/10.1038/s41420-025-02600-y]. Consequently, ApoE4 carriers may exist in a state of chronic metabolic compromise potentially rendering them susceptible to the significant surge in estrogen which occurs during the gestation period.

APOE4 and healthy cell bias theory:
- energy is required for ca2+ regulation
- estrogen is essential in regulating glucose transportation in brain and contributes to ca2+ homeostasis
- in healthy neurons, estrogen successfully enhances glucose metabolism and regulates Ca+ accumulation
- if injury is lready present and the ca+ regulartion has been disrupted, estrogen does not help anymore and exacerbates the disregulation
- APOE4 impairs glucose meatbolism and creates mitochondrial dysfunciton by impairing respiration in astrocytes, increasing ROS production and cause lipid accumulation
- lysosomal function is also impaired which is essential for CA2+ regulation
- this leads to previously mentioned neuronal injury which could get worse with estrogen

-> de lange study, circulating E2 and APOE4 interaction. Lee study

Using wildtype and APOE4 models middle aged female rats, Lee and colleagues show that APOE status modifies the effect of parity on neurogenesis and HV. Parity was associated with neurogenesis in wildtype rats, particularly in the ventral dentate gyrus, but decreased neurogenesis in APOE4 rats. In addition to this, parous APOE4 rats showed a spatial solving strategy which indicated a decreased recruitment of the hippocampus. To our knowledge, no study has explored this interaction of APOE4 and parity on hippocampal structure in humans.
...
CSF GFAP was assessed... CSF STREM... CSF YKL-40...
...
To test for potential confounders, analyses that showed a significant parity x APOE-e4 interaction were reproduced adding Aβ load as a covariate to disentangle the neuropathological contributions of Aβ accumulation from the phenogenetic contributions of APOE4. Given the high correlation between Aβ load and APOE4 variables, collinearity was tested for each model and can be found in the suppplementary materials. In addition to this, we reproduced the analysis in Aβ-negative participants only. After that, the following reproductive variables were also added as covariates to test if hormonal variables partly explained the effect at hand: HRT, early menopause, reproductive span and cause of menopause.

Additional exploratory mixed-effects linear regressions were performed to explore the interactives effects of parity APOE-e4 and time on hippocampal subfield volumes across visits. Subfields values were standardized for comparison purposes. Given the exploratory nature of these analyses, results are reported without correction for multiple comparisons. Lastly, we conducted similar linear regression using inflammatory biomarkers as outcomes to investigate their role in the observed association. These analyses were normalised using box-cox transformation to ensure that the distribution of residuals remained normal.
...
cognition_wide<-bind_rows(cognition1, cognition2)%>%
  drop_na(attention, executive, memory, language, visual)%>%
  group_by(subject)%>%
  filter(n()>1)%>%
  ungroup()%>%
...
data_table<-data_naked%>%
  ...
  dplyr::select(age_v1, kids_total, APOE_binary, education, vol_tiv_1, vol_HIPP_1, vol_HIPP_2, attention_V1, attention_V2, executive_V1, executive_V2, memory_V1, memory_V2, language_V1, language_V2, visual_V1, visual_V2, CSF_Abeta_ratio_V1, HRT, early_men, repro_span , cause_menopause,subject)%>%
  ...
  left_join(., diff_mri)%>%
  dplyr::select(age_v1, kids_total, education, APOE_binary, CSF_Abeta_ratio_V1, HRT, early_men, repro_span , cause_menopause, vol_HIPP_1 , diff_mri, attention_V1 , executive_V1, , memory_V1, language_V1, visual_V1, vol_tiv_1)
...
Carriers of APOE-E4 had significantly higher Aβ load compared with non carriers (t = 5, df = 191, p \< 0.001).

-   add biomarkers and menopause variables?
...
Parity, APOE-ε4, and time showed a significant three-way interaction for total HV (β = `r tidy(mod_ashs_APOE)$estimate[11]`, SE = `r tidy(mod_ashs_APOE)$std.error[11]`, p = `r tidy(mod_ashs_APOE)$p.value[11]`), indicating that higher parity was associated with steeper decline in HV among APOE-ε4 carriers (Figure X).

Stratifying by APOE-ε4 status yielded a significant interaction between parity and time in carriers only (β = `r tidy(mod_ashs_APOE_A1)$estimate[7]`, SE = `r tidy(mod_ashs_APOE_A1)$std.error[7]`, p = `r tidy(mod_ashs_APOE_A1)$p.value[7]`) (Figure X).
...
mod_ashs_APOE_ab_a0<-lmer(vol_HIPP ~  kids_total*diff_mri +age_v1 + scale(vol_tiv) + CSF_Abeta_ratio+ education  +(1|subject), data = data_ashs_female[data_ashs_female$APOE_binary==0,])
...
mod_ashs_APOE_horm<-lmer(vol_HIPP ~  kids_total*APOE_binary*diff_mri +age_v1 + scale(vol_tiv) +education  + repro_span+ early_men+ HRT+ cause_menopause + (1|subject), data = data_ashs_female)
summary(mod_ashs_APOE_horm)


```

To control for other potential confounding factors, the main analysis was first rerun including Aβ load as a covariate. The triple interaction effect was maintained when adjusting analyses by Aβ accumulation (β = `r tidy(mod_ashs_APOE_ab)$estimate[12]`, SE = `r tidy(mod_ashs_APOE_ab)$std.error[12]`, p = `r tidy(mod_ashs_APOE_ab)$p.value[12]`). However, Aβ load showed a notable association with the outcome (β = `r tidy(mod_ashs_APOE_ab)$estimate[7]`, SE = `r tidy(mod_ashs_APOE_ab)$std.error[7]`, p = `r tidy(mod_ashs_APOE_ab)$p.value[7]`), indicating it may play a role independently. When reproducing the analysis including only participants with negative Aβ status, the effect was maintained, although attenuated (CSF Aβ\<0.071 pg/ml, n = 128) (β = `r tidy(mod_ashs_APOE_abneg)$estimate[11]`, SE = `r tidy(mod_ashs_APOE_abneg)$std.error[11]`, p = `r tidy(mod_ashs_APOE_abneg)$p.value[11]`).

Interestingly, covarying the models stratified by APOE4 status for AB did not modify the effect of parity and time on HV in carriers (β = `r tidy(mod_ashs_APOE_ab_a1)$estimate[8]`, SE = `r tidy(mod_ashs_APOE_ab_a1)$std.error[8]`, p = `r tidy(mod_ashs_APOE_ab_a1)$p.value[8]`), but it yielded a signficant interaction of parity and time on HV in non-carriers (β = `r tidy(mod_ashs_APOE_ab_a0)$estimate[8]`, SE = `r tidy(mod_ashs_APOE_ab_a0)$std.error[8]`, p = `r tidy(mod_ashs_APOE_ab_a0)$p.value[8]`), which was not the case without adjusting for it (Figure X).

Additionally, replicating the analysis by further adjusting for hormonal variables (HRT, reproductive span, early menopause, cause of menopause) still yielded a significant 3-way estimate (β = `r tidy(mod_ashs_APOE_horm)$estimate[15]`, SE = `r tidy(mod_ashs_APOE_horm)$std.error[15]`, p = `r tidy(mod_ashs_APOE_horm)$p.value[15]`), suggesting these variables could not fully explain the association.
...
data_cog_composites<-bind_rows(cognition1, cognition2)%>%
  ...
  left_join(., A_long)%>%
  drop_na(attention, executive, memory, visual, language)%>%
  group_by(subject)%>%
  filter(n()>1)%>%
  ungroup()
  #filter(subject!= 21136)
...
The interaction between parity, APOE4 and time yielded a weak association with attention score, which was not present after FDR correction. None of the other scores showed any longitudinal association with our main predictors or 3-way interaction.
...
mod_ashs_CA1_a1<-lmer(vol_CA1_std ~  kids_total*diff_mri +age_v1  + scale(vol_tiv) + education +(1|subject), data = data_ashs_female[data_ashs_female$APOE_binary==1,])
...
mod_ashs_CA2_a1<-lmer(vol_CA2_std ~ kids_total*diff_mri + age_v1  + scale(vol_tiv) + education +(1|subject), data = data_ashs_female[data_ashs_female$APOE_binary==1,])
...
mod_ashs_DG_a1<-lmer(vol_DG_std ~  kids_total*diff_mri +age_v1  + scale(vol_tiv) + education + (1|subject), data = data_ashs_female[data_ashs_female$APOE_binary==1,])
...
After stratifying the analysis by APOE4 status, only the carrier group remained significant in the CA1 (β= `r tidy(mod_ashs_CA1_a1)$estimate[7]`, SE = `r tidy(mod_ashs_CA1_a1)$std.error[7]`, p= `r tidy(mod_ashs_CA1_a1)$p.value[7]`), CA2 (β= `r tidy(mod_ashs_CA2_a1)$estimate[7]`, SE = `r tidy(mod_ashs_CA2_a1)$std.error[7]`, p= `r tidy(mod_ashs_CA2_a1)$p.value[7]`) and DG (β= `r tidy(mod_ashs_DG_a1)$estimate[7]`, SE = `r tidy(mod_ashs_DG_a1)$std.error[7]`, p= `r tidy(mod_ashs_DG_a1)$p.value[7]`).

Adjusting whole group analysis by AB led to a stronger effect in the CA1 (β= `r tidy(mod_ashs_CA1_ab)$estimate[12]`, SE = `r tidy(mod_ashs_CA1_ab)$std.error[12]`, p= `r tidy(mod_ashs_CA1_ab)$p.value[12]`), but attenuated significance in the CA2 (β= `r tidy(mod_ashs_CA2_ab)$estimate[12]`, SE = `r tidy(mod_ashs_CA2_ab)$std.error[12]`, p= `r tidy(mod_ashs_CA2_ab)$p.value[12]`). Effects in the DG remained largely unchanged (β= `r tidy(mod_ashs_DG_ab)$estimate[12]`, SE = `r tidy(mod_ashs_DG_ab)$std.error[12]`, p= `r tidy(mod_ashs_DG_ab)$p.value[12]`).

## Exploratory analysis of inflammatory and cardiovascular biomarkers
...
gfap<-lmer(CSF_GFAP ~ kids_total*biomarkers_time_diff + age_v1  + CSF_Abeta_ratio+ education +(1|subject), data = data_biomarkers_long_female[data_biomarkers_long_female$APOE_binary==1,])
...
gfap2<-lmer(CSF_GFAP_transformed ~ kids_total*biomarkers_time_diff + age_v1  + CSF_Abeta_ratio+ education +(1|subject), data = data_biomarkers_long_female[data_biomarkers_long_female$APOE_binary==1,])
summary(gfap2) 
plot(gfap2)
...
strem<-lmer( CSF_sTREM2 ~ kids_total*biomarkers_time_diff + age_v1  + CSF_Abeta_ratio+ education +(1|subject), data = data_biomarkers_long_female[data_biomarkers_long_female$APOE_binary==1,])
...
strem2<-lmer(CSF_sTREM2_transformed ~ kids_total*biomarkers_time_diff + CSF_Abeta_ratio +age_v1  + education +(1|subject), data = data_biomarkers_long_female[data_biomarkers_long_female$APOE_binary==1,])
summary(strem2) 
plot(strem2)
...
yk40<-lmer( CSF_YKL40 ~ kids_total*biomarkers_time_diff + age_v1  + CSF_Abeta_ratio+ education +(1|subject), data = data_biomarkers_long_female[data_biomarkers_long_female$APOE_binary==1,])
...
yk402<-lmer(CSF_YKL40_transformed ~ kids_total*biomarkers_time_diff + CSF_Abeta_ratio+ age_v1  + education +(1|subject), data = data_biomarkers_long_female[data_biomarkers_long_female$APOE_binary==1,])
summary(yk402) 
plot(yk402)
qqnorm(residuals(yk402))

# does APOE seem to predict YKL levels?
ykl_apoe<-lmer(CSF_YKL40_transformed ~ APOE_binary*biomarkers_time_diff + age_v1 + CSF_Abeta_ratio + education +(1|subject), data = data_biomarkers_long_female)
plot(ykl_apoe)
summary(ykl_apoe)

```

Parity showed a weak association with YKL-40 in APOE4 carriers nearing significance, but this association was independent of time (β= `r tidy(yk402)$estimate[2]`, SE = `r tidy(yk402)$std.error[2]`, p= `r tidy(yk402)$p.value[2]`).
...
ggplot( aes(x = kids_total, y = diff_mri.trend)) +
  ...
  geom_line(data = all_slopes_cov, aes(x = kids_total, y = diff_mri.trend, color = APOE_binary), size = 1, linetype= "dashed")+
